...
export default function CheckoutPage() {
  ...
  const handlePayment = async () => {
    ...
    if (total <= 0) {
      alert("Order total must be greater than ₹0 to proceed with payment.");
      return;
    }
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input type="text" name="firstName" value={formData.firstName} onChange={handleInputChange} className="w-full bg-black border border-gray-800 rounded-xl py-3 px-4 text-white focus:border-blue-500 focus:outline-none transition-all" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-gray-400 ml-1 uppercase tracking-wider">Last Name</label>
                    <input type="text" name="lastName" value={formData.lastName} onChange={handleInputChange} className="w-full bg-black border border-gray-800 rounded-xl py-3 px-4 text-white focus:border-blue-500 focus:outline-none transition-all" />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <input type="text" name="address" value={formData.address} onChange={handleInputChange} placeholder="House no., Street, Area" className="w-full bg-black border border-gray-800 rounded-xl py-3 pl-12 pr-4 text-white placeholder-gray-600 focus:border-blue-500 focus:outline-none transition-all" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                  <div className="space-y-2">
                    ...
                    <input type="text" name="city" value={formData.city} onChange={handleInputChange} className="w-full bg-black border border-gray-800 rounded-xl py-3 px-4 text-white focus:border-blue-500 focus:outline-none transition-all" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-medium text-gray-400 ml-1 uppercase tracking-wider">State</label>
                    <input type="text" name="state" value={formData.state} onChange={handleInputChange} className="w-full bg-black border border-gray-800 rounded-xl py-3 px-4 text-white focus:border-blue-500 focus:outline-none transition-all" />
                  </div>
                  <div className="space-y-2 col-span-2 md:col-span-1">
                    <label className="text-xs font-medium text-gray-400 ml-1 uppercase tracking-wider">Pincode</label>
                    <input type="text" name="pincode" value={formData.pincode} onChange={handleInputChange} className="w-full bg-black border border-gray-800 rounded-xl py-3 px-4 text-white focus:border-blue-500 focus:outline-none transition-all" />
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="bg-[#121212] border border-gray-800 rounded-3xl p-6 md:p-8 relative overflow-hidden">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button
                  ...
                  className={`flex items-center p-4 rounded-xl border transition-all duration-300 relative overflow-hidden ${paymentMethod === 'phonepe' ? 'bg-[#5f259f]/20 border-[#5f259f] ring-1 ring-[#5f259f]' : 'bg-black border-gray-800 hover:border-gray-600'}`}
                >
                  <div className={`w-5 h-5 rounded-full border flex items-center justify-center mr-3 transition-colors ${paymentMethod === 'phonepe' ? 'border-[#5f259f]' : 'border-gray-600'}`}>
                    {paymentMethod === 'phonepe' && <div className="w-2.5 h-2.5 rounded-full bg-[#5f259f]" />}
                  </div>
                  <div className="flex flex-col items-start">
                    <div className="flex items-center">
                      <Smartphone className={`w-5 h-5 mr-2 transition-colors ${paymentMethod === 'phonepe' ? 'text-[#5f259f]' : 'text-gray-400'}`} />
                      <span className="font-medium">PhonePe / UPI</span>
                    </div>
                    {paymentMethod === 'phonepe' && <span className="text-[10px] text-gray-400 ml-7 mt-0.5">UPI, Cards, NetBanking</span>}
                  </div>
                  {paymentMethod === 'phonepe' && <div className="absolute right-0 top-0 w-16 h-16 bg-[#5f259f]/10 blur-xl rounded-full translate-x-1/2 -translate-y-1/2"></div>}
                </button>
            ...
            <div className="sticky top-28 space-y-6">
              ...
              <div className="bg-[#121212] border border-gray-800 rounded-3xl p-6 md:p-8 shadow-2xl shadow-black">
                ...
                <button
                  ...
                  className="w-full mt-8 bg-white text-black py-4 rounded-xl font-bold text-lg hover:bg-gray-200 active:scale-[0.98] transition-all shadow-xl shadow-white/5 flex items-center justify-center group disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isProcessing ? (
                    <>
