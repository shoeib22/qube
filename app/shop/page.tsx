          ...
          return (
            <section key={category} className="mb-20">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
                {items.map((p, index) => (
                  <Link
                    // ✅ FIX: composite key prevents duplicate-key crash
                    key={`${p.id}-${index}`}
                  ...
                  >
                    ...
                    <button
                      type="button"
                      className="mt-4 w-full px-4 py-2 bg-white text-black rounded-lg font-semibold hover:bg-gray-200 transition-colors"
                    >
                      View Details
                    </button>
                  </Link>
